...
// Слушаем обновление service worker'а СРАЗУ: пока показан экран доступа,
...
// Экран доступа поднимаем ДО boot(): так интерфейс монтируется уже
// заблокированным и ни на кадр не мелькает кликабельным. Сам интерфейс
// при этом рисуется по-настоящему — он служит фоном для карточки входа,
// видимым сквозь лёгкий блюр. Взаимодействовать с ним нельзя (inert +
// pointer-events: none), и сам по себе он ничего не делает: любое
// упражнение стартует только по кнопке, до которой не дотянуться.
const gate = hasAccess() ? null : mountGate();
// boot() в конце выставляет location.hash, а браузер на смену фрагмента
// сбрасывает фокус на body — поэтому фокус в поле кода возвращаем после
// того, как интерфейс домонтировался.
void boot().finally(() => gate?.focus());
